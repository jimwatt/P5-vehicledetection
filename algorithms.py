...
def getSVM(pf,trainSVM):
    if trainSVM:
        print("Training SVM ...")

        color_space = pf['color_space']
        spatial_size = pf['spatial_size']
        hist_bins = pf['hist_bins']
        orient = pf['orient']
        pix_per_cell = pf['pix_per_cell']
        cell_per_block = pf['cell_per_block']
        hog_channel = pf['hog_channel']
        spatial_feat = pf['spatial_feat']
        hist_feat = pf['hist_feat']
        hog_feat = pf['hog_feat']
        ...
        print("Number of non-cars : {}".format(len(notcars)))
        ...
        with open('svc.p', 'wb') as fp:
            pkl.dump((pf,svc,X_scaler,X_test,y_test),fp)

    else:
        print("\nLoading SVC ...")
        with open('svc.p', 'rb') as fp:
            pfsaved,svc,X_scaler,X_test,y_test = pkl.load(fp)
            if(pf!=pfsaved):     # need to make sure that we are using the same parameters that we trained with
                print('Parameter values have changed. Rerun training with new parameters (set trainSVM=True)')
                assert(pf==psaved)
    return svc,X_scaler,X_test,y_test

def find_cars(img, ystart, ystop, xstart, xstop, scale, step, pf, X_scaler, svc):
...
def accumulate_heat(heatmap, bbox_list):
...
def filt(a,b,alpha): # Smooth the car boxes
    newtrack = Track(alpha*a.xy + (1.0-alpha)*b.xy, alpha*a.vxy + (1.0-alpha)*b.vxy, alpha*a.sxy + (1.0-alpha)*b.sxy)
    return newtrack

def len_points(p1, p2): # Distance beetween two points
    return np.linalg.norm(p1.xy-p2.xy)
...
def adjust_aspect_ratio(sx,sy):
    min_ar = 1.0
    max_ar = 1.4
    ar = sx/sy 
    if(ar<min_ar): #box is too tall
        return (min_ar*sy,sy)
    elif(ar>max_ar):    # box is too flat
        return  (max_ar*sx,sx)
    else:
        return (sx,sy)
